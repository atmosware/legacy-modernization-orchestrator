...
| 3.5 | `delivery-planning` | Optional — offered after Phase 3 DoD passes; never blocking |
...
>
> **Phase 3.5 (`delivery-planning`) is optional and never blocking.** After Phase 3 DoD passes, the orchestrator **offers** it; if the business owner declines, proceed directly to Phase 4. It may run any time after Phase 3 and before or during Phase 4, and supports **standalone re-run** to refresh the plan when scope or tech stack changes. It reads Phase 1–3 artifacts (and `ui_design/` if Phase 4a already ran) and produces plans/estimates only — it never changes target-architecture decisions or gates Phase 4.
...
### `delivery-planning` _(Phase 3.5 — Optional, non-blocking)_
...
**Skill:** `.github/skills/delivery-planning/SKILL.md`  
